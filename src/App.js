...
function App() {
  ...
  const handleAllQuotes = async(author) =>{
    await fetch('http://api.quotable.io/quotes?author=' + author).then(res=>res.json()).then(res=>setAuthorData(res.results))
    setIsVisible(false)
  }
  useEffect(()=>{
    handleRandomQuote()
    ...
    (<div>    
          <div className='block mt-52 justify-center items-center flex-col'>
            <p className='text-xl w-4/12 mx-auto border-l-4 border-yellow-200 pl-12'>"{data.content}"</p>
              <div onClick={()=>handleAllQuotes(data.authorSlug)} className='bg-neutral-700 mt-12 mx-auto my-auto w-3/12 px-4 py-6 flex flex-row items-center justify-between hover:cursor-pointer'>
                <div className='text-white'>
                  <h1>{data.author}</h1>
                  <h3 className='text-xs text-neutral-500'>{data.tags ? data.tags[0] : ''}</h3>
                </div>
